...
content = content.replace(
  /<div className="space-y-1">\s*<label className="text-xs font-bold text-gray-700">In-service Date<\/label>\s*<input value=\{formData\.inServiceDate\}[^>]+>\s*<\/div>/s,
  newFields
);

fs.writeFileSync('src/VehicleDetailView.tsx', content);
console.log('Fixed buttons and modals again');
