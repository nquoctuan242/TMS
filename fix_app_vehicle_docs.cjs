...
// Add import
const importStatement = `import { VehicleDetailView } from './src/VehicleDetailView';\nimport { VehicleDocumentsView } from './src/VehicleDocumentsView';`;
content = content.replace(/import \{ VehicleDetailView \} from '\.\/src\/VehicleDetailView';/, importStatement);

// Add Sidebar Items
const oldFleetMenu = `<SidebarItem 
            icon="fa-truck" 
            label="Fleet" 
...
const newFleetMenu = `<SidebarItem 
            icon="fa-truck" 
            label="Fleet" 
...
content = content.replace(oldFleetMenu, newFleetMenu);

// Render Content
const renderContentReplace = `      {currentView === 'shift-control-detail' && <ShiftControlDetailView shiftControlId={selectedShiftControlId} onBack={() => setCurrentView('shift-control-list')} />}
...
content = content.replace(
  /\{currentView === 'shift-control-detail' && <ShiftControlDetailView shiftControlId=\{selectedShiftControlId\} onBack=\{\(\) => setCurrentView\('shift-control-list'\)\} \/>\}/,
  renderContentReplace
);
